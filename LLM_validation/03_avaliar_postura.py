        ...
        if response.status_code == 200:
            ...
            try:
                ...
                if "```json" in resposta_clean:
                    resposta_clean = (
                        resposta_clean.split("```json")[1].split("```")[0].strip()
                    )
                elif "```" in resposta_clean:
                    resposta_clean = (
                        resposta_clean.split("```")[1].split("```")[0].strip()
                    )
...
def processar_caso(pl_id, postura_esperada, resumo, discurso, modelos):
    ...
    for modelo_info in modelos:
        ...
        print_status(
            f"    {simbolo} {nome_display}: {resultado['postura_inferida']}", cor
        )
...
def main():
    ...
    for caso in casos_teste:
        ...
        for modelo_info in modelos:
            ...
            if not resumo:
                print_status(
                    f"  ⚠ Pulando {modelo_info['nome']}: resumo não encontrado", YELLOW
                )
                continue

            # Infere postura
            resultado = processar_caso(
                pl_id, postura_esperada, resumo, discurso, [modelo_info]
            )
