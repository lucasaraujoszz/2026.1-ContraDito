...
def resumir_ollama(model_name, prompt):
    ...
    try:
        ...
        return {"resumo": None, "tokens": 0, "tempo_segundos": 0, "erro": str(e)}
...
def processar_proposicao(pl_id, texto_completo, modelos):
    ...
    for modelo_info in modelos:
        ...
        if resultado["erro"]:
            ...
            print_status(
                f"    ✓ {resultado['tokens']} tokens em {resultado['tempo_segundos']}s",
                GREEN,
            )
...
def main():
    ...
    for modelo_info in modelos:
        ...
        print_status(
            f"  → {sucessos}/{total} resumos gerados com sucesso",
