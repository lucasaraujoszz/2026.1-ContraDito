...
def main():
    ...
    for slug, dados in scores_brutos.items():
        score = (
            ...
            + dados["acuracia"]
            * PESOS["justificativa"]  # proxy: acurácia representa qualidade
    ...
    for i, (slug, score) in enumerate(ranking[:3]):
        ...
        cor = cor_nota(score)
        print(
            f"  {medalhas[i]}  {cor}{BOLD}{nome:<24}{RESET}  Score: {cor}{score}{RESET}"
        )
    ...
    locais = [
        s
        for s in ranking
        if scores_brutos[s[0]]["nome"] in ["Llama 3.1 8B", "Qwen 2.5 7B", "Gemma 2 9B"]
    ]
    ...
    if melhor_local:
        ...
        print(
            f"  {GREEN}Resumo (tarefa simples):{RESET} {BOLD}{nome_local}{RESET} (melhor local — sem limite de requisições)"
        )
