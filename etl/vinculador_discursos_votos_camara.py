# Módulo para vinculação de chunks de discursos próximos aos votos (Câmara dos Deputados)
import logging
...
def filtrar_chunks_validos(chunks_qdrant: list, threshold: float = 0.72, limit: int | None = None) -> list[dict]:
    """
...
def resolver_textos_chunks(
    ...
    # Mapeia ID -> texto
    text_map = {row["id"]: row["texto_chunk"] for row in data if "id" in row and "texto_chunk" in row}
...
def _buscar_embedding_proposicao(qdrant_client, proposicao_id: str) -> list[float] | None:
    ...
    try:
        resultado = qdrant_client.scroll(
            ...
            scroll_filter=Filter(
                must=[
                    FieldCondition(key="proposicao_id_string", match=MatchValue(value=proposicao_id)),
                    FieldCondition(key="casa", match=MatchValue(value="camara"))
                ]
            ),
            with_vectors=True,
            limit=1,
        ...
        logger.error(f"Erro ao buscar embedding da proposicao {proposicao_id} no Qdrant: {e}")
        return None


def executar_pipeline_vinculo_camara(
    supabase_client,
    qdrant_client,
    ...
    """
    Orquestra o vínculo de chunks aos votos da Câmara dos Deputados de forma incremental.
    ...
    try:
        ...
        while True:
            query = (
                supabase_client.table("camara_votos")
        ...
        for voto in votos:
            ...
            if not query_vector:
                logger.warning(f"Voto {id_voto}: embedding não encontrado para proposição {proposicao_id}. Ignorando.")
                continue
                
            # Busca chunks mais próximos no Qdrant para o deputado
            ...
            # Filtra e ordena usando regras de domínio (sem limite superior)
            chunks_com_score = filtrar_chunks_validos(points, threshold=threshold, limit=None)
            ...
            supabase_client.table("camara_votos").update({
