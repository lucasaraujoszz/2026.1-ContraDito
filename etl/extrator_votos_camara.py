...
async def obter_id_votacao_camada_1(
    client: httpx.AsyncClient, id_camara_proposicao: int
...
async def obter_id_votacao_camada_2_sweep(
    client: httpx.AsyncClient, id_camara_proposicao: int
) -> Optional[Tuple[str, Optional[str]]]:
    """
    ...
    for votacao in votacoes:
        ...
        url_votos = (
            f"https://dadosabertos.camara.leg.br/api/v2/votacoes/{id_votacao}/votos"
        )
...
async def extrair_votos_proposicao(
    ...
    try:
        ...
        if not resultado_busca:
            resultado_busca = await obter_id_votacao_camada_2_sweep(
                client, id_camara_proposicao
            )
        ...
        votos_transformados = [
            transformar_voto_camara(voto, proposicao_id) for voto in votos_validos
        ]
        ...
        logger.error(
            f"Falha isolada (Poison Pill) ao extrair votos da proposição {proposicao_id}: {e}"
        )
        return [], None, None


async def executar_pipeline_votos_camara(
    ...
    try:
        ...
        resp_politicos = (
            supabase_client.table("camara_politicos").select("id").execute()
        )
        ids_validos_banco = (
        ...
        query_proposicoes = supabase_client.table("camara_proposicoes").select(
            "proposicao_id, id_camara"
        )
        if limite_amostral:
        ...
        async with httpx.AsyncClient() as client:
                ...
                async with semaphore:
                    ...
                    try:
                        votos, id_votacao, data_votacao = (
                            await extrair_votos_proposicao(
                                client, proposicao_id, id_camara, ids_validos_banco
                            )
                        )
            ...
            for res in resultados:
                ...
                if id_votacao:
                    ...
                    supabase_client.table("camara_proposicoes").update(
                        payload_update
            ...
            if votos_totais_upsert:
                ...
                votos_deduplicados = list(
                    {v["id"]: v for v in votos_totais_upsert}.values()
                )
                ...
                logger.info(
                    f"Upsert concluído: {total_linhas} votos validados salvos na base."
                )
