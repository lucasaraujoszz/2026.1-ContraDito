...
async def test_obter_id_votacao_camada_1_caminho_feliz():
    ...
    mock_json = {
        "dados": [
            {"id": "voto-falso", "descricao": "Rejeitado o Requerimento."}, # Ignorar (Falso Positivo)
            {"id": "voto-alvo", "descricao": "Aprovada a PEC. Sim: 340; Não: 110; Abstenção: 7; Total: 457.", "dataHoraRegistro": "2023-10-05T15:30:00"} # Match perfeito da Regex
        ]
    }
    ...
    async with httpx.AsyncClient() as client:
        ...
        assert resultado == ("voto-alvo", "2023-10-05T15:30:00"), "O extrator deve retornar a tupla com ID e Data."
...
async def test_obter_id_votacao_camada_2_sweep():
    ...
    respx.get(url_votacoes).respond(
        status_code=200, 
        json={"dados": [{"id": "sessao-1"}, {"id": "sessao-2", "dataHoraRegistro": "2023-10-05T16:00:00"}]}
    )
    respx.get("https://dadosabertos.camara.leg.br/api/v2/votacoes/sessao-1/votos").respond(status_code=200, json={"dados": []})
    respx.get("https://dadosabertos.camara.leg.br/api/v2/votacoes/sessao-2/votos").respond(status_code=200, json={"dados": [{"deputado_": {"id": 10}}]})
    
    async with httpx.AsyncClient() as client:
        resultado = await obter_id_votacao_camada_2_sweep(client, id_camara_proposicao)
        assert resultado == ("sessao-2", "2023-10-05T16:00:00"), "A Camada 2 deve retornar a tupla com ID e Data."
...
async def test_obter_id_votacao_camada_2_sweep_ignora_falsos_positivos():
    ...
    respx.get(url_votacoes).respond(
        ...
        json={"dados": [
            ...
            {"id": "sessao-merito", "descricao": "Aprovada a PEC.", "dataHoraRegistro": "2023-10-05T17:00:00"}
        ]}
    )
    respx.get("https://dadosabertos.camara.leg.br/api/v2/votacoes/sessao-requerimento/votos").respond(status_code=200, json={"dados": [{"deputado_": {"id": 10}}]})
    respx.get("https://dadosabertos.camara.leg.br/api/v2/votacoes/sessao-merito/votos").respond(status_code=200, json={"dados": [{"deputado_": {"id": 20}}]})
    
    async with httpx.AsyncClient() as client:
        resultado = await obter_id_votacao_camada_2_sweep(client, id_camara_proposicao)
        assert resultado == ("sessao-merito", "2023-10-05T17:00:00"), "A Camada 2 deve pular o requerimento e retornar a votação de mérito."
...
async def test_obter_id_votacao_camada_1_resiliencia_retry():
    ...
    mock_json = {
        "dados": [{"id": "voto-alvo", "descricao": "Aprovada a PEC. Sim: 100; Não: 10", "data": "2023-10-05"}]
    }
...
async def test_extrair_votos_proposicao_poison_pill():
    ...
    async with httpx.AsyncClient() as client:
        votos, id_votacao, data_votacao = await extrair_votos_proposicao(client, proposicao_id, id_camara, ids_validos)
        
        assert votos == [], "Deve retornar lista vazia de votos em caso de falha crítica."
...
async def test_extrair_votos_proposicao_caminho_feliz():
    ...
    url_votacoes = f"https://dadosabertos.camara.leg.br/api/v2/proposicoes/{id_camara}/votacoes"
    mock_votacoes = {
        "dados": [{"id": "voto-feliz", "descricao": "Aprovada a PEC. Sim: 340; Não: 110", "dataHoraRegistro": "2023-10-05T15:30:00"}]
    }
    ...
    mock_votos = {
        "dados": [
            ...
            {"deputado_": {"id": 99, "siglaPartido": "PSDB"}, "tipoVoto": "Não"}, # Fantasma (Soft Drop)
    ...
    async with httpx.AsyncClient() as client:
        ...
        assert len(votos) == 2, "O suplente fantasma (99) deve ter sido limpo pelo Soft Drop."
...
async def test_executar_pipeline_votos_camara_update_duplo(mock_extrair):
    ...
    mock_extrair.return_value = ([{"id": "voto1", "politico_id": 10}], "sessao-alvo", "2023-10-05T15:30:00")
    ...
    mock_update.assert_called_with({
        "id_votacao_camara": "sessao-alvo",
        "data_votacao": "2023-10-05T15:30:00"
    })
