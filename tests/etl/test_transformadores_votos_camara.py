...
from etl.transformadores_votos_camara import eh_votacao_merito_nominal, gerar_id_voto_camara, filtrar_votos_validos, transformar_voto_camara

@pytest.mark.parametrize("descricao, esperado", [
    # --- IGNORAR (Falsos Positivos / Votações Procedimentais) ---
    ("Rejeitado o Requerimento. Sim: 102; Não: 322; Abstenção: 1; Total: 425.", False),
    ("Aprovada a preferência. Sim: 467; Não: 4; Abstenção: 1; Total: 472.", False),
    ("Aprovado o Requerimento nº 4.491/2024, dos Senhores Líderes, que solicita a quebra de interstício de 5 sessões previsto no § 6º do art. 202 do RICD, para apreciação do segundo turno da PEC 5, de 2023.", False),
    ("Aprovada a Redação Final assinada pelo relator, Dep. Dr. Fernando Máximo (UNIÃO-RO).", False),
    
    # --- APROVAR (Votações Nominais de Mérito Verdadeiras) ---
    ("Aprovada, em primeiro turno, a Proposta de Emenda à Constituição nº 5, de 2023, na forma da Emenda Aglutinativa Substitutiva nº 3. Sim: 385; Não: 93; Abstenção: 7; Total: 485.", True),
    ("Mantido o texto. Sim: 340; Não: 110; Abstenção: 7; Total: 457.", True),
    ("Aprovada, em segundo turno, a Proposta de Emenda à Constituição nº 5, de 2023. Sim: 368; Não: 96; Abstenção: 7; Total: 471.", True),
])
...
def test_transformar_voto_camara_sucesso():
    ...
    """
    voto_bruto = {
        "deputado_": {"id": 74646, "siglaPartido": "PSDB"},
        "tipoVoto": "Sim"
    }
    proposicao_id = "pec_5_2023"
