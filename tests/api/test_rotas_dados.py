...
def test_obter_politico_detalhado_sucesso(client, mock_supabase):
    ...
    mock_query_resumo.select.return_value.eq.return_value.eq.return_value.execute.return_value = mock_resumo_result
...
def test_obter_chunks_discurso_sucesso(client, mock_supabase):
    ...
    resposta = client.get("/api/camara/discursos/e0e84b6f-7023-4554-949e-f00de7a44f77/chunks")
    assert resposta.status_code == 200
...
def test_obter_proposicao_detalhada_sucesso(client, mock_supabase):
    ...
    resposta = client.get("/api/camara/proposicoes/c0c1c2c3-d4d5-e6e7-f8f9-a0a1a2a3a4a5")
    assert resposta.status_code == 200
...
def test_listar_votos_sucesso(client, mock_supabase):
    ...
    resposta = client.get("/api/camara/votos?politico_id=12345&proposicao_id=PL 123/2026")
    assert resposta.status_code == 200
    dados = resposta.json()
    assert dados["total_registros"] == 1
    assert dados["itens"][0]["id"] == "d0d1d2d3-e4e5-f6f7-a8a9-b0b1b2b3b4b5"
    assert dados["itens"][0]["voto_oficial"] == "Sim"
    assert dados["itens"][0]["chunks_proximos"][0]["texto_chunk"] == "educação é prioridade"
